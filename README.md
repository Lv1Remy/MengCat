...
零依赖、开箱即用的网页小黑猫：眼睛（白眼球+黑瞳孔分层）跟随鼠标、按下变 `> <`、点击 Q 弹、拖拽拉长/甩歪（松手回弹）、闲置 1 分钟睡着（`- -` 眼 + 冒 Z）、随机眨眼、软边剪影。
...
| `> <` 表情 | 按住期间 | 眼睛换成括号 polyline（尖角朝中间），松开后滞留 320ms |
| 点击 Q 弹 | 松手且未拖拽时 | WAAPI 从当前形态无缝接续 `scale(1.07,0.88)→(0.97,1.06)→(1,1)` |
| 拖拽拉长 | 按住向上拖 | 阻力渐增（越拉越拉不动），横向体积守恒变细，底部固定；松手多段 Q 弹甩回 |
| 拖拽甩歪 | 按住左右拖 | skewX 斜切（底边不动），松手左右回摆衰减 |
| 睡眠 | 闲置 1 分钟 | 眼睛变 `- -`，右上角冒 Z（循环上飘淡出），呼吸放缓；任意鼠标/键盘活动立即醒来（Q 弹一下） |
| 眨眼 | 闲置 2.6~5s 随机 | WAAPI `scaleY 1→0.08→1`，170ms（睡着时不眨） |
| 呼吸 | 常驻 | `scaleY 1↔1.028`，3.4s 循环（睡着放缓到 5.6s） |
...
- `SLEEP_AFTER` —— 闲置多久入睡（默认 60000ms）
- `MAX_STRETCH` / `STRETCH_RESIST` —— 拉长上限与阻力
- `MAX_LEAN` / `LEAN_RESIST` —— 左右斜切上限与阻力
